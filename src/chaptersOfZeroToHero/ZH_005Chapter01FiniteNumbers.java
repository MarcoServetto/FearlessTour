...
- The inductive case is used if the left number is not zero.
If the number is not zero we can see the left number `this` as 
the result of `this.pred.succ`.
We can then split the `this.pred` and the `.succ` parts. 
We then move this `.succ` to the right of the `+` operator.
Putting those ideas together we get the code of `Number+`:
```
+(other: Number): Number -> this.pred + (other.succ)
```
This reasoning is based on the idea that it is very easy to sum a number with zero,
and that we can slowly reduce any addition to a sum with zero by moving units around.


We now show the reduction for `3 + 2`.
Note how in the first step `this` is `3` (not `0`) and `other` is `2`.
Thus we apply method `Number+`.
...
In step 10, `this` is `0`, and thus we apply method `0+`.

This inductive approach, defining a base case and a rule to reduce other cases
...
Again, think of `this` as `this.pred.succ`. We can apply the mathematical property 
`(this.pred.succ) * other = (this.pred * other) + other`.
Multiplying `this` by `other` is the same as first multiplying `this.pred` by `other`,
and then adding `other` to that result. This reduces the multiplication problem
`this * other` to a smaller multiplication `this.pred * other` plus an addition
(`+ other`), eventually reaching the base case where the left side is `0`.
...
Note how here we do get `7` as we would expect with the normal precedence of
...
Subtraction `-` presents a slight twist. The easy base case is `this - 0 = this`,
where zero is on the right. However, our inductive approach works by simplifying
the left operand `this`. How can we handle subtraction?

We can use a helper method. The main `-` method can delegate to an auxiliary method
(let's call it `._rightSub`) that effectively swaps the operands so the induction
can work correctly based on the original right-hand operand.
...
We've meticulously built our own `Number` type, representing the numbers `0`
through `11`, complete with `.succ`, `.pred`, and arithmetic operations.
...
- Third party libraries, that can be written by any Fearless programmer and have to be
  manually included into our projects.
- The Fearless standard library, which is integrated with Fearless and is always
...
How does the standard library provide all eighteen quintillion `Nat` types?
Did someone actually type them all out?
...
just the definitions for `Nat` would fill a stack of books so high...
it would stretch roughly from the Earth to Pluto. Seriously.
